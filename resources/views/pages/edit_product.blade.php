                            ...
                            <div class="col-sm-5 no-padding-left">
                                ...
                                <div class="form-group col-sm-12 no-padding">
                                    ...
                                    <select class="form-control" id="unit" name="unit">
                                        ...
                                        @foreach($units as $unit)
                                            <option value="{{$unit->id}}" @if($product->unit == $unit->id) selected @endif>{{$unit->name}} ({{$unit->code}})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="clearfix"></div>
                                <div class="form-group col-sm-12 no-padding">
                                    ...
                                    <select class="form-control" id="sale_unit" name="sale_unit">
                                        <option>select unit</option>
                                        @foreach($units as $unit)
                                            <option value="{{$unit->id}}" @if($product->sale_unit == $unit->id) selected @endif >{{$unit->name}} ({{$unit->code}})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="clearfix"></div>
                                <div class="form-group col-sm-12 no-padding">
                                    ...
                                    <select class="form-control" id="purchase_unit" name="purchase_unit">
                                        <option>select unit</option>
                                        @foreach($units as $unit)
                                            <option value="{{$unit->id}}" @if($product->purchase_unit == $unit->id) selected @endif>{{$unit->name}} ({{$unit->code}})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="clearfix"></div>
                                ...
                                <div class="tab-content">
                                    <div id="details" class="tab-pane fade in active">
                                        ...
                                        <div class="form-group @if ($errors->has('details')) has-error @endif">
                                            ...
                                            <input type="text" name="details" value="{!! nl2br($product->details) !!}" class="form-control">
                                    ...
                                    <div id="productPhoto" class="tab-pane fade">
                                        ...
                                        <div class="form-group @if($errors->has('image')) has-error @endif">
                                            <img src="{{asset('stock/assets/uploads/'.$product->image)}}">
                                            <div class="clearfix"></div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <table class="table table-responsive table-striped table-bordered table-hover">
                                                ...
                                                @foreach($thumbnails as $thumb)
                                                    <tr class="row_thumb">
                                                        <td><img src="{{asset('stock/assets/uploads/thumbs/'.$thumb->photo)}}"> </td>
                                                        <td><a style="cursor: pointer" class="delete_thumb_admin" id="{{$thumb->id}}"><i class="fa fa-trash"></i> </a> </td>
                                                    </tr>
                                                @endforeach
                                                </tbody>
                                            </table>
                                        ...
                                        <script type="text/javascript">
                                            var baseUrl = "{{ url('/'.$product->id) }}";
                                            var token = "{{ Session::token() }}";
                                            Dropzone.autoDiscover = false;
                                            var myDropzone = new Dropzone("div#dropzoneFileUpload", {
                                                url: baseUrl + "/admin/thumbnail/uploadFiles",
                                                params: {
                                                    _token: token
                                                }
                                            });
