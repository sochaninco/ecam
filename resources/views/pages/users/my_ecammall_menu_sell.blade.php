        ...
        <div class="category-tab hidden-xs"><!--category-tab-->
            <div class="col-sm-12 no-padding">
                <ul class="nav nav-tabs">
                    ...
                    <li class="@yield('my_customer_order')-tab" style="width: 50%;font-weight: bold"><a href="@if($ShopInfo) {{url('em-user/'.$userId.'/customer_orders')}} @else {{url('#')}} @endif">Sell</a></li>
                </ul>
            </div>
        </div>
        <div class="panel-group category-products" id="accordian"><!--category-products-->
                <div class="panel panel-default">
                    <div class="panel-heading @yield('my_customer_order')-tab">
                        <h4 class="panel-title"><a href="{{url('em-user/'.$userId.'/my_shop_info')}}">My Shop Information</a></h4>
                    </div>
                </div>
                <div class="panel panel-default">
                    <div class="panel-heading @yield('my_customer_order')-tab">
                        <h4 class="panel-title">
                            <a href="@if($ShopInfo) {{url('em-user/'.$userId.'/customer_orders')}} @else {{url('#')}} @endif">Customer Orders</a>
                        </h4>
                    </div>
                </div>
                <div class="panel panel-default">
                <div class="panel-heading @yield('my_customer_order')-tab">
                    <h4 class="panel-title"><a href="@if($ShopInfo) {{url('em-user/'.$userId.'/my_promotion')}} @else {{url('#')}} @endif">My Promotion</a></h4>
                </div>
            </div>
                <div class="panel panel-default">
                    <div class="panel-heading @yield('my_customer_order')-tab">
                        <h4 class="panel-title"><a href="{{url('#')}}">Manage Feedback</a></h4>
                    </div>
                </div>
                <div class="panel panel-default">
                    <div class="panel-heading @yield('my_customer_order')-tab">
                        <h4 class="panel-title"><a href="@if($ShopInfo) {{url('em-user/'.$userId.'/my_coupons')}} @else {{url('#')}} @endif">My Coupons</a></h4>
                    </div>
                </div>
                <div class="panel panel-default">
                <div class="panel-heading @yield('my_customer_order')-tab">
                    <h4 class="panel-title"><a href="{{url('em-user/'.$userId.'/my_shipping_address')}}">My Shipping Address</a></h4>
                </div>
            </div>
                <div class="panel panel-default">
                <div class="panel-heading @yield('my_customer_order')-tab ">
                    <h4 class="panel-title"><a href="@if($ShopInfo) {{url('em-user/'.$userId.'/shop_member')}} @else {{url('#')}} @endif">Shop Member</a></h4>
                </div>
            </div>
        </div>
