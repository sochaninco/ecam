...
@section('content')
    ...
    <section id="advertisement">
        <div class="container no-padding">
                <?php
                $Product_banner = \App\CategorySlide::where(['status'=>0,'slide_type'=>8])->get();
                ?>
    ...
    <div class="container white-bg">
        <div class="row">
                                            ...
                                            <div class="single-products">
                                                <div class="productinfo-collection text-center">
                                                    <a href="{{url('product_detail/'.$item->id)}}">
                                                        <img alt="" src="{{$base_url}}stock/assets/uploads/{{$item->image}}" class="brand-product img-responsive">
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="product-image-wrapper">
                                            <div class="single-products">
                                                <div class="brand-logo-scroll text-center">
                                                    <a href="{{url('products/brands/'.$brand->id)}}"><img src="{{$base_url}}stock/assets/uploads/{{$brand->image}}" alt="" /></a>
            ...
            @foreach($latestBrand as $brands)
                <?php
                    $checkProductBrand = \App\ShopProduct::where('brand',$brands->id)->count();
                ?>
                @if($checkProductBrand > 3)
                    <div class="col-sm-3 padding-bottom text-center border hidden-xs">
                        <a href="{{url('products/brands/'.$brands->id)}}">
                        <div class="no-padding">
                            @if(empty($brands->banner))
                            <img src="{{$base_url}}stock/assets/uploads/{{$brands->image}}" class="img-responsive banner-brand">
                            @else
                                <img src="{{asset('images/brand/banner/'.$brands->banner)}}" class="img-responsive ">
                            @endif
                        </div>
                        <div class="logo-box">
                            <img class="img-brand-zone" src="{{$base_url}}stock/assets/uploads/{{$brands->image}}">
                        </div>
                        </a>
                        ...
                        <?php
                            $products = \App\ShopProduct::where('brand',$brands->id)->take(3)->get();
                        ?>
                        ...
                        <div class="col-md-4 col-sm-4 col-xs-4 no-padding">
                            <div class="text-center">
                                <a href="{{url('shop/product_detail/'.$product->id)}}">
                                    {{--<img alt="" src="http://heangsochan.com/stock/assets/uploads/{{$product->image}}" class="brand-product">--}}
                    ...
                    <div class="col-sm-3 text-center visible-xs">
                            <a href="{{url('products/brands/'.$brands->id)}}">
                                <div class="no-padding">
                                    @if(empty($brands->banner))
                                        <img src="{{$base_url}}stock/assets/uploads/{{$brands->image}}" class="img-responsive banner-brand" style="width: 384px">
                                    @else
